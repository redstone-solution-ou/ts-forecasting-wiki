...
| Chronos-2 | MASE skill, WQL skill, SQL skill, median runtime | [GIFT-Eval](../datasets-benchmarks/gift-eval.md) (97 tasks), Chronos Bench II (27), fev-bench (100) | zero-shot; 95% bootstrap CIs | TiRex, TimesFM-2.5, Toto, Moirai-2.0, TabPFN-TS, Sundial, COSMIC, sNaive | Yes (9-quantile decoder) | [../papers/chronos-2.md](../papers/chronos-2.md) |
...
| LaT-PFN | MSE, CRRMSE (forecasting); accuracy (classification SVM probe over fixed-length summaries) | ECL, ETTh1, ETTh2, Illness, Traffic (forecasting); UCR-128 (classification) | zero-shot — synthetic-only pretraining, evaluation never sees real data during training | ARIMA, ForecastPFN, FBProphet (forecasting); TS2Vec (classification, trained on UCR) | No (point-only via categorical decoder over 100 bins) | [../papers/lat-pfn.md](../papers/lat-pfn.md) |
| TS-JEPA | accuracy (classification); MSE, MAE (forecasting; cumulative MSE for long-term) | FordA, FordB, FaultDetectionA, FaultDetectionB, ECG5000 (classification); Weather, ETT-Small, Electricity (forecasting) | self-supervised pretrain on the dataset's own training set, then frozen-encoder + small downstream head | TS2Vec (contrastive), MAE (input-space masked), autoregressive, randomly-initialized frozen encoder, fully supervised CNN + transformer | No (point only) | [../papers/ts-jepa.md](../papers/ts-jepa.md) |
| MTS-JEPA | F1, AUC, Precision, Recall (window-level anomaly *prediction*) | MSL, SMAP, SWaT, PSM | self-supervised pretrain on official train set; downstream classifier trained on 6:2:2 chronological split of test set; 5 random seeds | K-Means, DeepSVDD, LSTM-VAE, TS2Vec, TimesNet, PatchTST, iTransformer, PAD, TS-JEPA | No (binary anomaly classification head over window-level codes) | [../papers/mts-jepa.md](../papers/mts-jepa.md) |
