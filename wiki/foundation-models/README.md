...
- **Universal evaluation.** Corpora like the Monash archive, LOTSA,
  the Time-Series Pile, Time-300B, TimeBench and GIFT-Eval exist
...
- **Early 2024 cohort.** TimesFM (Google), Chronos (Amazon), MOMENT
  (CMU), MOIRAI (Salesforce) and Timer (Tsinghua) appeared within a
  ...
  benchmarks.
- **MoE scaling (late 2024).** Time-MoE and Moirai-MoE pushed total
  parameter counts above a billion while keeping activated parameters
  manageable, echoing the mixture-of-experts move in NLP.
- **Long context and true multivariate (late 2024 to 2025).** Timer-XL
  and Chronos-2 extended context lengths and added first-class
  multivariate conditioning, a significant step beyond the
  channel-independent defaults of the early cohort.
- **Continuous / generative (2025).** Sundial introduced flow-matching
...
frozen LLMs for time series (Time-LLM, GPT4TS, LLMTime), and a
lightweight thread (TTM, Mamba4Cast) showed that very small models can
...
- **Tokenization.** Patch embeddings (TimesFM, MOIRAI, MOMENT,
  ...
  learned vector-quantized tokens (TOTEM, Sundial). See
...
- **Task scope.** Forecast-only (the majority) versus multi-task
  ...
  detection from a single pretrained checkpoint (UniTS, TOTEM, MOMENT,
  Timer-XL, Chronos-2).
...
- [../concepts/README.md](../concepts/README.md) — cross-cutting
  ...
  in-context learning, scaling laws, RevIN, multi-task, synthetic
  data).
